...
const totalRooms: number = 0

let treasureRooms: number[] = [2, 4, 7]
let monstersRooms: number[] = [3, 6, 8]

for (let room = 1; room <= totalRooms; room++) {
    let haveTreasure = treasureRooms.includes(room)
    let haveMonsters = monstersRooms.includes(room)
}
