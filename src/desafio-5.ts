...
function choosingPokemon (chosenNumber: number) {
    if (chosenNumber == 1){
        const chosenPokemon = "Bulbasaur"
        return chosenPokemon
    } else if (chosenNumber == 2) {
        const chosenPokemon = "Charmander"
        return chosenPokemon
    } else if (chosenNumber == 4) {
        const chosenPokemon = "Pikachu"
        return chosenPokemon
    } else if (chosenNumber == 5) {
        const chosenPokemon = "Mewtwo"
        return chosenPokemon
    } else {return console.error("Número de seleção indisponível");
    }
}

console.log(choosingPokemon(3))